...
} from '../_deps/constants.js';
import { unlockAudio } from '../_deps/notificationSound.js';
import { transport } from '../_deps/transport.js';
...
import type { CollabLink, RenderContext } from './CollabOverlay.js';
import { CollabOverlay } from './CollabOverlay.js';
import type { AgentTaskStatus } from '../_deps/webviewStubs.js';
import { TaskBadgeOverlay } from './TaskBadgeOverlay.js';
import { DepartmentOverlay } from './DepartmentOverlay.js';
...
interface OfficeCanvasProps {
  ...
  /** Collaboration links to visualise as animated lines. */
  collabLinks?: CollabLink[];
  /** Task status badges to render above agent heads. */
  taskStatuses?: Map<number, AgentTaskStatus>;
  /** Designated boss/orchestrator agent id — persistent crown */
  bossAgentId?: number | null;
  /** agentId → area label, for the floating department chips */
  agentAreas?: Record<number, string | null>;
  /** Open a department detail panel when a chip is clicked */
...
export function OfficeCanvas({
  ...
  return (
    <div ref={containerRef} className="w-full h-full relative overflow-hidden bg-bg">
      ...
      {((taskStatuses && taskStatuses.size > 0) || bossAgentId != null) && (
        <TaskBadgeOverlay
          officeState={officeState}
          taskStatuses={taskStatuses ?? new Map()}
          renderContextRef={renderContextRef}
          bossAgentId={bossAgentId}
        />
      )}
      {onOpenDepartment && (
        <DepartmentOverlay
