...
function logValidationStatus(provider: string, status: number): void {
  logger.info(`[agentcorp-validate] ${provider} HTTP ${status}`);
}
...
function logValidationRequest(
  ...
  logger.info(
    `[agentcorp-validate] ${provider} request ${method} ${sanitizeValidationUrl(url)} headers=${JSON.stringify(sanitizeHeaders(headers))}`,
  );
}
...
async function validateOpenAiCompatibleKey(
  ...
  if (modelsResult.status === 404) {
    logger.info(
      `[agentcorp-validate] ${providerType} /models returned 404, falling back to ${apiProtocol} probe`,
    );
    if (apiProtocol === 'openai-responses') {
...
async function validateAnthropicHeaderKey(
  ...
  if (
    ...
    logger.info(
      `[agentcorp-validate] ${providerType} /models returned error, falling back to /messages probe`,
    );
