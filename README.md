...
- **可执行验证**：代码题的 `code_runnability` 不看模型怎么说，看代码怎么跑。
...
开启后，`code` 工种的试做题会把候选写的测试真的跑一遍，产出
`verifiedEvidence`（如「沙盒执行：4/4 用例通过（212ms）」），
这是唯一能解除 `code_runnability` 权重折减的证据。
...
- **沙盒验证覆盖的是「能不能跑」，不是「安不安全」**。`code_runnability`
  已由真实执行背书（`SANDBOX_ENABLED=true` 开启），但 `code_security`
  仍缺真实扫描，因此它继续被降权 ×0.4 —— 接入静态扫描前我们不会假装它被验证过。
  另外沙盒提供的是**资源限制 + 隔离目录 + 超时强杀**，不提供网络与文件系统隔离；
